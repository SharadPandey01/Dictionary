...
export default function Home() {
    ...
    const getMeaning = async () => {
        ...
        if (!trimmedWord){
            toast.warn("Please enter a word to search!");
            return;
        }
    ...
    return(
        ...
        <div className="h-full w-full flex flex-col items-center justify-center gap-3 bg-[#010409]">
            <div className="HomeSearchSection flex  w-[75%] h-fit border border-gray-500  focus-within:border-black focus-within:ring-3 focus-within:ring-white">
                <input className='h-full w-full bg-white text-black text-lg' type="text" name="HomeSearchWord" value={word} onChange={(e) => {setWord(e.target.value)}} onKeyDown={(e) => { if (e.key === "Enter") getMeaning(); }} placeholder=" Search any word... " />
